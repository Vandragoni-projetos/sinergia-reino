...
error_log("GatewayPro_TRACK_ENDPOINT: Script iniciado.");
...
try {
    ...
    error_log("GatewayPro_TRACK_ENDPOINT: Dados brutos recebidos: " . $raw_post_data);
    ...
    if (json_last_error() !== JSON_ERROR_NONE || !is_array($data)) {
        ...
        error_log("GatewayPro_TRACK_ENDPOINT: Erro ao decodificar JSON ou dados vazios/inválidos: " . json_last_error_msg() . ". Dados brutos: " . $raw_post_data);
    ...
    foreach ($required_fields as $field) {
        if (empty($data[$field])) {
            http_response_code(400);
            error_log("GatewayPro_TRACK_ENDPOINT: Campo obrigatório ausente: '$field'. Dados recebidos: " . print_r($data, true));
    ...
    if (!in_array($event_type, $allowed_event_types)) {
        http_response_code(400);
        error_log("GatewayPro_TRACK_ENDPOINT: Tipo de evento inválido: '$event_type'. Dados recebidos: " . print_r($data, true));
    ...
    $stmt_tracking_product = $pdo->prepare("SELECT id FROM gatewaypro_tracking_products WHERE tracking_id = :tracking_id");
    ...
    if (!$tracking_product_id_db) {
        ...
        error_log("GatewayPro_TRACK_ENDPOINT: Tracking ID inválido ou não encontrado: '$tracking_id'. Dados recebidos: " . print_r($data, true));
        echo json_encode(['success' => false, 'error' => 'Tracking ID inválido.']);
        exit;
    }

    // 4. Inserir o evento na tabela GatewayPro_tracking_events
    ...
    if ($event_type === 'page_view' || $event_type === 'initiate_checkout') {
        error_log("GatewayPro_TRACK_ENDPOINT: Verificando evento existente para '$event_type' na sessão '$session_id' para tracking_product_id '$tracking_product_id_db'.");
        $check_existing_stmt = $pdo->prepare("SELECT COUNT(*) FROM GatewayPro_tracking_events WHERE tracking_product_id = :tracking_product_id AND session_id = :session_id AND event_type = :event_type");
        ...
        if ($check_existing_stmt->fetchColumn() > 0) {
            error_log("GatewayPro_TRACK_ENDPOINT: Evento '$event_type' já registrado para a sessão '$session_id' e tracking_product_id '$tracking_product_id_db'. Ignorando inserção.");
            $insert_event = false; // Não insere se já existe
        }
    }

    if ($insert_event) {
        $stmt_insert = $pdo->prepare("INSERT INTO GatewayPro_tracking_events (tracking_product_id, session_id, event_type, event_data) VALUES (:tracking_product_id, :session_id, :event_type, :event_data)");
        ...
        if ($stmt_insert->execute()) {
            error_log("GatewayPro_TRACK_ENDPOINT: Evento '$event_type' registrado com sucesso. Session ID: '$session_id', Tracking Product DB ID: '$tracking_product_id_db'.");
            ob_clean(); // Limpa o buffer antes de enviar o JSON
            echo json_encode(['success' => true, 'message' => 'Evento registrado com sucesso.']);
        } else {
            http_response_code(500);
            error_log("GatewayPro_TRACK_ENDPOINT: Erro ao registrar evento no banco de dados. SQLSTATE: " . $stmt_insert->errorCode() . ", Detalhes: " . print_r($stmt_insert->errorInfo(), true) . ". Dados recebidos: " . print_r($data, true));
    ...
    http_response_code(500);
    error_log('GatewayPro_TRACK_ENDPOINT: Erro Fatal: ' . $e->getMessage() . ' no arquivo ' . $e->getFile() . ' na linha ' . $e->getLine());
    ob_clean(); // Limpa o buffer antes de enviar o JSON
