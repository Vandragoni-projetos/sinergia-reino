...
function trigger_webhooks($usuario_id, $event_data, $trigger_event, $produto_id = null) {
    ...
    foreach ($webhooks as $url) {
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
            ...
            'X-GatewayPro-Event: ' . $trigger_event, // Cabeçalho personalizado
            'User-Agent: GatewayProWebhookAgent/1.0' // User-Agent personalizado
        ]);
...
function trigger_utmfy_integrations($usuario_id, $event_data, $trigger_event, $produto_id = null, $mercado_pago_payment_data = []) {
    ...
    // Mapeamento de status da GatewayPro para status da UTMfy
    $status_map = [
        'approved'     => 'paid',
    ...
    // Mapeamento de métodos de pagamento da GatewayPro para UTMfy
    $payment_method_map = [
        'Pix' => 'pix',
        'Boleto' => 'boleto',
    ...
    foreach ($integrations as $api_token) {
        ...
        foreach ($event_data['produtos_comprados'] as $product) {
            ...
            $products_payload[] = [
                ...
                'planId' => null, // Assumindo que GatewayPro não tem 'planId'
                'planName' => null, // Assumindo que GatewayPro não tem 'planName'
        ...
        if (empty($created_at_utc)) {
            ...
            log_webhook("UTMFY: data_venda da GatewayPro vazia, usando tempo atual UTC para createdAt.");
        }


        $payload = [
            'orderId' => (string)$event_data['transacao_id'],
            'platform' => 'GatewayProBR6', // Nome da sua plataforma
            ...
            'commission' => [
                ...
                'gatewayFeeInCents' => 0, // Ajustar se a GatewayPro tiver uma taxa de gateway fixa/conhecida
...
function process_single_product_delivery($product_data, $customer_email) {
    ...
    log_webhook("  Iniciando processamento de entrega para produto '$product_name'. Tipo: '$delivery_type'. Oferta ID: " . ($oferta_id ?? 'NULL'));
        ...
        case 'area_membros':
            if (!empty($customer_email) && !empty($product_id_for_area_membros)) {
                ...
                if (function_exists('conceder_acesso_aluno')) {
                    ...
                    if ($acesso_concedido) {
                        log_webhook("    SUCESSO DE ENTREGA (Área de Membros): Acesso concedido via helper para " . $customer_email . " ao produto ID " . $product_id_for_area_membros . " (oferta: " . ($oferta_id ?? 'padrão') . ")");
                        return ['success' => true, 'product_name' => $product_name, 'content_type' => 'area_membros', 'content_value' => null];
                    } else {
                        log_webhook("    ERRO DE ENTREGA (Área de Membros): Falha ao conceder acesso via helper para " . $customer_email);
                        return ['success' => false, 'message' => "Falha ao conceder acesso para o produto '$product_name'."];
                    }
                } else {
                    // Fallback: usa INSERT com oferta_id se o helper não estiver disponível
                    ...
                    if ($stmt_grant_access->rowCount() > 0) {
                        log_webhook("    SUCESSO DE ENTREGA (Área de Membros): Acesso concedido para " . $customer_email . " ao produto ID " . $product_id_for_area_membros . " (oferta: " . ($oferta_id ?? 'NULL') . ")");
                        return ['success' => true, 'product_name' => $product_name, 'content_type' => 'area_membros', 'content_value' => null];
                    } else {
                                        ...
                                        if ($delivery_result['content_type'] === 'area_membros') {
                                            if ($member_area_password_for_delivery === null) {
                                                ...
                                                if ($existing_user) {
                                                    ...
                                                    // Apenas atualiza o nome se necessário
                                                    $stmt_update_user = $pdo->prepare("UPDATE usuarios SET nome = ? WHERE id = ?");
                                                    $stmt_update_user->execute([$customer_name, $existing_user['id']]);
                                                    log_webhook("  Área de Membros: Usuário existente encontrado, senha mantida: " . $customer_email);
                                                    // Envia mensagem informativa no e-mail
                                                    $member_area_password_for_delivery = '(use a mesma senha cadastrada anteriormente)';
                                                } else {
