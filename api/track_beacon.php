...
log_beacon("GatewayPro_TRACK_BEACON: Script iniciado.");

try {
    require_once __DIR__ . '/../config/config.php';
    log_beacon("GatewayPro_TRACK_BEACON: config.php carregado com sucesso.");
    ...
    if (empty($tracking_id) || empty($session_id) || empty($event_type)) {
        log_beacon("GatewayPro_TRACK_BEACON: Erro: tracking_id, session_id ou event_type ausente. tracking_id: " . ($tracking_id ?? 'N/A') . ", session_id: " . ($session_id ?? 'N/A') . ", event_type: " . ($event_type ?? 'N/A'));
    ...
    if (!$tracking_product_id_db) {
        log_beacon("GatewayPro_TRACK_BEACON: Tracking ID inválido ou não encontrado: $tracking_id.");
        ob_end_clean(); // Descarta qualquer saída do buffer
        exit;
    }
    ...
    if ($raw_event_data) {
        ...
        if (json_last_error() === JSON_ERROR_NONE && is_array($decoded_raw_event_data)) {
            ...
            log_beacon("GatewayPro_TRACK_BEACON: event_data mesclado. Dados finais: " . json_encode($event_data));
        } else {
            log_beacon("GatewayPro_TRACK_BEACON: raw_event_data inválido ou não JSON: '$raw_event_data'. Ignorando mesclagem.");
        }
    }
    ...
    if ($check_existing_stmt->fetchColumn() > 0) {
        log_beacon("GatewayPro_TRACK_BEACON: Evento '$event_type' já registrado para a sessão '$session_id' e tracking_product_id '$tracking_product_id_db'. Ignorando.");
        ob_end_clean(); // Descarta qualquer saída do buffer
        exit;
    }
    ...
    if ($stmt_insert->execute()) {
        log_beacon("GatewayPro_TRACK_BEACON: Evento '$event_type' registrado com sucesso. Session ID: '$session_id', Tracking Product DB ID: '$tracking_product_id_db'.");
    } else {
        log_beacon("GatewayPro_TRACK_BEACON: Erro ao registrar evento no banco de dados. SQLSTATE: " . $stmt_insert->errorCode() . ", Detalhes: " . print_r($stmt_insert->errorInfo(), true));
    }

} catch (Throwable $e) { // Captura Exception e Error
    log_beacon('GatewayPro_TRACK_BEACON: Erro Fatal: ' . $e->getMessage() . ' no arquivo ' . $e->getFile() . ' na linha ' . $e->getLine());
}
