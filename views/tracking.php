...
// Verifica se a tabela gatewaypro_tracking_products existe no banco
$table_exists = false;
try {
    ...
    error_log("Erro ao verificar existência da tabela gatewaypro_tracking_products: " . $e->getMessage());
    $table_exists = false;
}
...
try {
    if (isset($pdo) && $pdo instanceof PDO) {
        ...
        if ($table_exists) {
            try {
                $stmt_tracked_products = $pdo->prepare("SELECT stp.id, stp.produto_id, stp.tracking_id, p.nome FROM gatewaypro_tracking_products stp JOIN produtos p ON stp.produto_id = p.id WHERE stp.usuario_id = :usuario_id ORDER BY p.nome ASC");
...
<div class="container mx-auto">
    <h1 class="text-3xl font-bold text-white mb-6">GatewayPro Track</h1>
    ...
    <div class="bg-dark-card p-8 rounded-lg shadow-md mb-8 border border-[#32e768] <?php echo !$tracking_allowed ? 'opacity-50 pointer-events-none' : ''; ?>">
        ...
        <div class="space-y-6">
            <div>
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:space-x-4 space-y-4 sm:space-y-0">
                    <select id="product_select" class="w-full sm:w-2/3 px-4 py-3 bg-dark-elevated border border-dark-border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#32e768] text-white">
                    ...
                    <button id="add_track_product_btn" class="w-full sm:w-1/3 bg-[#32e768] text-white font-bold py-3 px-6 rounded-lg hover:bg-[#28d15e] transition duration-300 flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed">
    ...
    <div id="tracking_data_section" class="bg-dark-card p-8 rounded-lg shadow-md border border-[#32e768]" style="display: none;">
        <div class="flex items-center justify-between mb-6 border-b border-dark-border pb-4">
            <div>
                <h2 class="text-2xl font-semibold text-white">Análise de Funil para <span id="analyzed_product_name" class="text-[#32e768]"></span></h2>
        ...
        <div class="flex flex-wrap items-center justify-start gap-3 mb-6 bg-dark-elevated p-2 rounded-lg border border-dark-border">
            ...
            <button data-period="today" class="period-filter-btn px-3 py-1 text-sm font-semibold rounded-md bg-[#32e768] text-white shadow">Hoje</button>
        ...
        <div class="flex flex-col lg:flex-row gap-8 mb-8">
            <div class="lg:w-1/2 w-full bg-dark-elevated p-6 rounded-lg border border-dark-border shadow-sm">
                <h3 class="text-xl font-semibold text-white mb-4 flex items-center space-x-2"><i data-lucide="funnel" class="w-6 h-6 text-[#32e768]"></i><span>Funil de Vendas</span></h3>
            ...
            <div class="lg:w-1/2 w-full space-y-4">
                ...
                <div class="bg-green-900/20 border-l-4 border-green-500 p-4 rounded-lg shadow-sm">
                 ...
                 <div class="bg-[#32e768]/20 border-l-4 border-[#32e768] p-4 rounded-lg shadow-sm">
                    <p class="text-gray-400 text-sm">Taxa de Conversão Geral</p>
                    <p id="conversion_overall" class="text-3xl font-bold text-[#32e768]">0%</p>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let currentTrackingProductDbId = null; // ID da tabela gatewaypro_tracking_products
    ...
    addTrackProductBtn.addEventListener('click', async function() {
        ...
        try {
            const response = await fetch('/api/api?action=add_GatewayPro_tracked_product', {
                method: 'POST',
    ...
    async function refreshTrackedProductsList() {
        try {
            const response = await fetch('/api/api?action=get_GatewayPro_tracked_products');
            const result = await response.json();
    ...
    trackedProductsList.addEventListener('click', async function(e) {
        ...
        if (generateBtn) {
            ...
            try {
                const response = await fetch(`/api/api?action=generate_tracking_script&tracking_id=${trackingId}`);
                const result = await response.json();
    ...
    async function fetchTrackingData(trackingProductDbId, period) {
        try {
            const response = await fetch(`/api/api?action=get_GatewayPro_tracking_data&tracking_product_id=${trackingProductDbId}&period=${period}`);
            const result = await response.json();
    ...
    trackedProductsList.addEventListener('click', async function(e) {
        ...
        if (deleteBtn) {
            ...
            if (confirm(`Tem certeza que deseja excluir o funil de rastreamento para o produto "${productName}"? Esta ação não pode ser desfeita.`)) {
                try {
                    const response = await fetch('/api/api?action=delete_GatewayPro_tracked_product', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
