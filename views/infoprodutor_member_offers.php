        ...
        if (isset($_POST['active_offers']) && is_array($_POST['active_offers'])) {
            ...
            foreach ($_POST['active_offers'] as $offer_product_id) {
                ...
                if ($stmt_check_offer_product->rowCount() > 0) {
                    // Pega o link personalizado e texto do botão, se fornecidos
                    $custom_link = isset($_POST['custom_links'][$offer_product_id]) ? trim($_POST['custom_links'][$offer_product_id]) : null;
                    $custom_link = !empty($custom_link) ? $custom_link : null;
                    $custom_button_text = isset($_POST['custom_button_texts'][$offer_product_id]) ? trim($_POST['custom_button_texts'][$offer_product_id]) : null;
                    $custom_button_text = !empty($custom_button_text) ? $custom_button_text : null;
                    $stmt_insert->execute([$source_product_id, $offer_product_id, $custom_link, $custom_button_text]);
                }
            }
        }
...
// 4. Buscar as ofertas ativas existentes para este source_product_id (incluindo custom_link e custom_button_text)
$stmt_current_offers = $pdo->prepare("
    SELECT offer_product_id, custom_link, custom_button_text
    FROM product_exclusive_offers
    WHERE source_product_id = ? AND is_active = 1
");
$stmt_current_offers->execute([$source_product_id]);
$current_offers_data = $stmt_current_offers->fetchAll(PDO::FETCH_ASSOC);

// Criar arrays para fácil acesso
$current_offer_product_ids = array_column($current_offers_data, 'offer_product_id');
$current_offer_custom_links = [];
$current_offer_button_texts = [];
foreach ($current_offers_data as $offer) {
    $current_offer_custom_links[$offer['offer_product_id']] = $offer['custom_link'];
    $current_offer_button_texts[$offer['offer_product_id']] = $offer['custom_button_text'];
}

// 5. Combinar os dados: Adicionar flags 'is_offer', 'custom_link' e 'custom_button_text' a cada produto potencial
foreach ($potential_offer_products as &$product) {
    $product['is_offer'] = in_array($product['id'], $current_offer_product_ids);
    $product['custom_link'] = $current_offer_custom_links[$product['id']] ?? '';
    $product['custom_button_text'] = $current_offer_button_texts[$product['id']] ?? '';
}
            ...
            <form action="/index?pagina=infoprodutor_member_offers&source_product_id=<?php echo $source_product_id; ?>" method="post">
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    <?php foreach ($potential_offer_products as $product): ?>
                        <div class="group bg-dark-elevated rounded-lg overflow-hidden border border-dark-border hover:shadow-lg transition-shadow duration-300 flex flex-col offer-card" data-product-id="<?php echo $product['id']; ?>">
                            ...
                            <div class="p-4 flex-grow flex flex-col justify-between">
                                ...
                                <div class="mt-4 space-y-3">
                                    ...
                                    <label for="offer-<?php echo $product['id']; ?>" class="flex items-center cursor-pointer">
                                        <div class="relative">
                                            <input type="checkbox" id="offer-<?php echo $product['id']; ?>" name="active_offers[]" value="<?php echo $product['id']; ?>" class="sr-only peer offer-toggle" <?php echo $product['is_offer'] ? 'checked' : ''; ?>>
                                    ...
                                    <!-- Campo de link personalizado (aparece quando oferta está ativa) -->
                                    <div class="custom-link-container <?php echo $product['is_offer'] ? '' : 'hidden'; ?>">
                                        ...
                                        <input 
                                            type="url" 
                                            name="custom_links[<?php echo $product['id']; ?>]" 
                                            value="<?php echo htmlspecialchars($product['custom_link']); ?>" 
                                            placeholder="https://exemplo.com/oferta"
                                            class="w-full px-3 py-2 bg-dark-card border border-dark-border rounded-lg text-white text-sm placeholder-gray-500 focus:outline-none focus:border-[#32e768] transition-colors"
                                        >
                                        <p class="text-xs text-gray-500 mt-1">Deixe vazio para usar o checkout padrão</p>
                                        
                                        <!-- Campo de texto do botão -->
                                        <label class="block text-xs text-gray-400 mb-1 mt-3">
                                            <i data-lucide="type" class="w-3 h-3 inline-block mr-1"></i>
                                            Texto do botão (opcional)
                                        </label>
                                        <input 
                                            type="text" 
                                            name="custom_button_texts[<?php echo $product['id']; ?>]" 
                                            value="<?php echo htmlspecialchars($product['custom_button_text']); ?>" 
                                            placeholder="Ex: Garantir Acesso, Saiba Mais..."
                                            maxlength="100"
                                            class="w-full px-3 py-2 bg-dark-card border border-dark-border rounded-lg text-white text-sm placeholder-gray-500 focus:outline-none focus:border-[#32e768] transition-colors"
                                        >
                                        <p class="text-xs text-gray-500 mt-1">Deixe vazio para usar "Comprar por R$ X,XX"</p>
                                    </div>
                                </div>
                            </div>
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Toggle para mostrar/esconder campo de link personalizado
        document.querySelectorAll('.offer-toggle').forEach(function(toggle) {
            toggle.addEventListener('change', function() {
                const card = this.closest('.offer-card');
                const linkContainer = card.querySelector('.custom-link-container');
