...
<div class="container mx-auto">
    <div class="flex items-center mb-6">
        ...
        <div>
            ...
            <p class="text-gray-400">Conecte a GatewayPro à UTMfy para enviar notificações de vendas e rastrear links.</p>
        </div>
    </div>
    ...
    <div class="bg-dark-card p-8 rounded-lg shadow-md mb-8 border border-[#32e768]">
        ...
        <form id="utmfy-form" class="space-y-6">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="event_approved" class="form-checkbox h-5 w-5 text-green-400 rounded focus:ring-[#32e768]">
                        <span class="ml-2 text-gray-300">Compra Aprovada</span>
                    </label>
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="event_pending" class="form-checkbox h-5 w-5 text-yellow-400 rounded focus:ring-[#32e768]">
                        <span class="ml-2 text-gray-300">Pagamento Pendente</span>
                    </label>
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="event_rejected" class="form-checkbox h-5 w-5 text-red-400 rounded focus:ring-[#32e768]">
                        <span class="ml-2 text-gray-300">Pagamento Recusado</span>
                    </label>
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="event_refunded" class="form-checkbox h-5 w-5 text-purple-400 rounded focus:ring-[#32e768]">
                        <span class="ml-2 text-gray-300">Reembolso</span>
                    </label>
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="event_charged_back" class="form-checkbox h-5 w-5 text-pink-400 rounded focus:ring-[#32e768]">
                        <span class="ml-2 text-gray-300">Chargeback</span>
                    </label>
                    <label class="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="event_initiate_checkout" class="form-checkbox h-5 w-5 text-indigo-400 rounded focus:ring-[#32e768]">
            ...
            <div class="flex items-center justify-end space-x-4 mt-6">
                ...
                <button type="submit" id="save-utmfy-btn" class="bg-[#32e768] text-white font-bold py-2 px-5 rounded-lg hover:bg-[#28d15e] transition duration-300 flex items-center space-x-2">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchUtmfyIntegrations() {
        showLoading();
        try {
            const response = await fetch('/api/api?action=get_utmfy_integrations');
    ...
    async function saveUtmfyIntegration(action, integrationData) {
        try {
            const response = await fetch(`/api/api?action=${action}`, {
                method: 'POST',
    ...
    async function deleteUtmfyIntegration(integrationId) {
        ...
        try {
            const response = await fetch('/api/api?action=delete_utmfy_integration', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
