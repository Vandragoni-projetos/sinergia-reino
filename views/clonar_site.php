...
<div class="container mx-auto p-4 lg:p-8 max-w-7xl">
    ...
    <div class="bg-dark-card rounded-xl shadow-lg border border-[#32e768] overflow-hidden mb-8 transition-all hover:shadow-xl">
        <div class="p-6 md:p-8">
            <h2 class="text-xl font-bold text-white mb-4 flex items-center">
                <div class="p-2 bg-[#32e768]/20 rounded-lg mr-3">
                    <i data-lucide="globe" class="w-5 h-5 text-[#32e768]"></i>
                </div>
                Nova Clonagem
            </h2>
                ...
                <div class="flex flex-col md:flex-row gap-4 items-end">
                    <div class="flex-grow w-full">
                        ...
                        <div class="relative rounded-md shadow-sm">
                            ...
                            <input type="url" id="url_to_clone" name="url_to_clone" class="focus:ring-[#32e768] focus:border-[#32e768] block w-full pl-16 sm:text-sm border-dark-border rounded-lg py-3 bg-dark-elevated text-white" placeholder="www.exemplo.com/pagina" required>
                        </div>
                    </div>
                    <button type="submit" class="w-full md:w-auto flex items-center justify-center py-3 px-6 border border-transparent text-sm font-medium rounded-lg text-white bg-[#32e768] hover:bg-[#28d15e] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#32e768] transition-colors shadow-md hover:shadow-lg shadow-[#32e768]/20">
    ...
    <div id="editorSection" class="hidden bg-dark-card rounded-xl shadow-lg border border-[#32e768] overflow-hidden mb-8">
        <div class="border-b border-dark-border bg-[#32e768]/10 p-6">
            <div class="flex flex-col lg:flex-row justify-between lg:items-center gap-6">
                ...
                <div class="flex-grow space-y-4 w-full lg:w-2/3">
                    <div>
                        ...
                        <input type="text" id="clonedSiteTitle" class="block w-full border-dark-border bg-dark-elevated rounded-md shadow-sm focus:ring-[#32e768] focus:border-[#32e768] sm:text-lg font-medium p-2 text-white" placeholder="Ex: Página de Vendas Black Friday">
                    </div>
...
<script>
    ...
    cloneForm.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch('/api/api?action=clone_url', {
                method: 'POST',
    ...
    saveVisualEditorBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch('/api?action=save_cloned_site', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
    ...
    async function loadClonedSites() {
        try {
            const response = await fetch('/api?action=get_cloned_sites');
            const data = await response.json();
    ...
    async function editClonedSite(id) {
        ...
        try {
            const response = await fetch(`/api?action=get_cloned_site_details&cloned_site_id=${id}`);
            const data = await response.json();
    ...
    async function deleteClonedSite(id) {
        ...
        try {
            const response = await fetch('/api?action=delete_cloned_site', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
