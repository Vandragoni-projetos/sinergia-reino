...
// Validar e buscar o produto (ID no POST ou no GET)
...
// Aviso se POST veio vazio (limite do servidor)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($_POST) && !empty($_SERVER['CONTENT_LENGTH']) && (int)$_SERVER['CONTENT_LENGTH'] > 0) {
    $mensagem = "<div class='bg-amber-900/30 border border-amber-500 text-amber-200 px-4 py-3 rounded relative mb-4' role='alert'><strong>Formulário não recebido.</strong> O envio pode ter sido bloqueado (tamanho muito grande). Tente imagens menores ou aumente post_max_size/upload_max_filesize no PHP.</div>";
}

// Processar formulário de salvamento
$is_save_post = isset($_POST['salvar_checkout']) || ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id_produto']) && isset($_POST['backgroundColor']));
if ($is_save_post) {
