<div class="container mx-auto">
    ...
    <div class="bg-gradient-to-br from-purple-800 via-indigo-900 to-black p-6 rounded-lg shadow-md mb-6 text-white relative overflow-hidden">
        ...
        <div class="relative z-10">
            <h2 class="text-3xl font-extrabold text-white mb-4">Sua Jornada GatewayPro</h2>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Mapeamento das etapas da jornada GatewayPro
    // COMENTADO - Não será usado por enquanto
    /*
    const GatewayPro_STAGES = [
        { name: 'ESTRELA CADENTE', min: 0, max: 10000, description: 'Toda grande jornada começa com um vislumbre. Você acendeu a sua luz no universo GatewayPro. É o ponto de partida de uma trajetória que promete ser épica.', color: 'from-purple-800 via-indigo-900 to-black' },
        { name: 'NEBULOSA ASCENDENTE', min: 10000, max: 100000, description: 'Uma formação se inicia, cores vibrantes e um brilho crescente. Você está moldando seu espaço na galáxia GatewayPro, com potencial para se expandir e impressionar.', color: 'from-indigo-900 via-blue-950 to-gray-900' },
        ...
        { name: 'SUPERNOVA DOURADA', min: 500000, max: 1000000, description: 'Um evento cósmico de proporções grandiosas! Sua energia explode, iluminando vastas extensões. Você é uma força que redefine o seu entorno, uma referência inconfundível.', color: 'from-yellow-400 via-amber-600 to-green-700' },
        { name: 'GALÁXIA SUPREMA', min: 1000000, max: 5000000, description: 'Você se tornou um universo em si, com trilhões de estrelas e uma complexidade que inspira. Uma galáxia construída com visão, resiliência e paixão, reconhecida por sua vastidão.', color: 'from-green-400 via-red-600 to-rose-700' },
    ...
    function renderGatewayProJourney(totalFaturamentoLifetime) {
        ...
        for (let i = 0; i < GatewayPro_STAGES.length; i++) {
            if (totalFaturamentoLifetime >= GatewayPro_STAGES[i].min && totalFaturamentoLifetime < GatewayPro_STAGES[i].max) {
                ...
                nextStage = GatewayPro_STAGES[i + 1] || null; 
                break;
            }
        }
        ...
        if (!currentStage && totalFaturamentoLifetime >= GatewayPro_STAGES[GatewayPro_STAGES.length - 1].min) {
            currentStage = GatewayPro_STAGES[GatewayPro_STAGES.length - 1]; // Last stage
            nextStage = null; 
        } else if (!currentStage) { 
             currentStage = GatewayPro_STAGES[0]; // Default to first stage if below its min
             nextStage = GatewayPro_STAGES[1];
        }
    ...
    */
    ...
    function updateKpiAndChart(period = 'today') {
        ...
        fetch(`/api/api?action=get_dashboard_data&period=${period}`)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                ...
                // Renderiza a Jornada GatewayPro com o faturamento lifetime
                // COMENTADO - Não será usado por enquanto
                // renderGatewayProJourney(data.kpis.total_faturamento_lifetime);
                ...
                const chartConfig = {
                    ...
                    data: {
                        ...
                        datasets: [{
                            ...
                            tension: 0.4,
                            fill: true,
                            pointBackgroundColor: '#2DD05E',
                            pointBorderColor: '#fff',
                            pointHoverBackgroundColor: '#fff',
                            pointHoverBorderColor: '#2DD05E',
